    ...
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold {{ $activeServices > 0 ? 'text-green-400' : 'text-white' }} mt-1">{{ $activeServices }}</p>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-white">سرویس‌های فعال</h3>
        ...
        @else
            <div class="space-y-3">
                ...
                @endphp
                <a href="{{ route('dashboard.services.show', $service) }}" class="flex items-center justify-between bg-gray-800/50 hover:bg-gray-800 rounded-lg px-4 py-3 transition">
                    <div>
                        <div class="text-sm font-medium text-white">{{ $service->plan_name ?? 'سرویس VPN' }}</div>
                        <div class="text-xs text-gray-500 mt-0.5 font-mono">{{ $service->service_number }}</div>
                    </div>
                    <div class="text-left">
                        <div class="text-xs {{ $statusColor }}">{{ $service->statusLabel() }}</div>
                        @if($service->expires_at)
                        <div class="text-xs text-gray-500 mt-0.5">{{ $service->expires_at->format('Y/m/d') }}</div>
                        @endif
                    </div>
                </a>
                @endforeach
