...
<div class="bg-gray-900/60 border-b border-gray-800 text-xs text-gray-400">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-10 flex items-center justify-between gap-3 flex-wrap">
        ...
        <span class="hidden sm:inline">{{ site_setting('topbar_guarantee_text', '۷ روز ضمانت بازگشت وجه') }}</span>
    </div>
</div>
...
<header class="sticky top-0 z-50 bg-gray-900/85 backdrop-blur border-b border-gray-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-2">
                @auth
                    <a href="{{ route('dashboard.index') }}"
                       class="zed-btn inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold bg-gray-800 text-cyan-400 border border-gray-700 hover:border-cyan-400 transition">
                ...
                @else
                    ...
                    <a href="{{ route('register') }}" class="zed-btn zed-btn-primary px-4 py-2 text-sm font-semibold">ثبت‌نام</a>
                @endauth

                <button id="modern-menu-btn" class="lg:hidden text-gray-400 hover:text-white p-2" aria-label="منو">
        ...
        {{-- Mobile menu --}}
        <div id="modern-menu" class="hidden lg:hidden pb-4 space-y-1">
            @foreach($navLinks as $link)
...
<script>
    document.getElementById('modern-menu-btn')?.addEventListener('click', function () {
        document.getElementById('modern-menu')?.classList.toggle('hidden');
    });
</script>
@stack('scripts')
</body>
