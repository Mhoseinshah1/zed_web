...
{{-- ===== Main nav (site-wide; mobile hamburger required) ===== --}}
        ...
        <nav class="flex items-center justify-between h-16">
            <a href="{{ route('home') }}" class="flex items-center gap-2.5 font-extrabold text-xl text-white">
                @if($logo = cms_image('logo'))
                    @include('partials.site-logo', ['src' => $logo, 'class' => 'h-8 w-auto', 'style' => 'min-height:2rem', 'eager' => true])
                @else
            ...
            <div class="flex items-center gap-2">
                ...
                <button id="map-menu-btn" class="lg:hidden text-gray-400 hover:text-white p-2" aria-label="منو">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
                </button>
            </div>
        </nav>

        <div id="map-menu" class="hidden lg:hidden pb-4 space-y-1">
            @foreach($navLinks as $link)
