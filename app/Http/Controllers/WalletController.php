...
class WalletController extends Controller
{
    ...
    public function topupForm()
    {
        ...
        $presetAmounts = collect(explode(',', SiteText::get('wallet_topup_preset_amounts', '50000,100000,200000,500000')))
            ->map(fn ($v) => (int) trim($v))
            ->filter(fn ($v) => $v > 0)
            ->values()
            ->all();

        $minAmount = max(1, (int) SiteText::get('wallet_min_topup_amount', '10000'));
        $maxAmount = max($minAmount, (int) SiteText::get('wallet_max_topup_amount', '10000000'));
